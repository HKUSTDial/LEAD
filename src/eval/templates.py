...
def create_prompt_with_qwen2_7b_format(messages, tokenizer, add_bos=True):
    """
    Create a formatted prompt string for Qwen-2.7B based on the provided messages.

    Args:
        messages (list): A list of message dictionaries, each containing 'role' and 'content' fields.
        tokenizer: The tokenizer instance (e.g., QwenTokenizer).
        add_bos (bool): Whether to add the beginning-of-sequence token at the start of the prompt.

    Returns:
        str: The formatted prompt string.
    """
    # Define Qwen-specific delimiters
    ...
    formatted_text += assistant_start

    # Add beginning-of-sequence token if required
    if add_bos:
        formatted_text = tokenizer.bos_token + formatted_text
